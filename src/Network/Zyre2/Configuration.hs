...
setName zctx@(ZyreContext ptr _) name = unlessStale zctx $ do
...
setHeader zctx@(ZyreContext ptr _) headerName headerValue = unlessStale zctx $ do
...
setEvasiveTimeout zctx@(ZyreContext ptr _) timeout = unlessStale zctx $ do
...
setSilentTimeout zctx@(ZyreContext ptr _) timeout = unlessStale zctx $ do
...
setExpiredTimeout zctx@(ZyreContext ptr _) timeout = unlessStale zctx $ do
...
setInterval zctx@(ZyreContext ptr _) interval = unlessStale zctx $ do
...
setInterface zctx@(ZyreContext ptr _) interface = unlessStale zctx $ do
